...
var effect = require("../effect_rock");
...
describe("Effect Rock Logic", function() {
    ...
    describe(".ApplyEffect with invalid effect type", function() {
        it("should throw an exception for an invalid effect type", function(){
            ...
            }).should.throw('Effect of type rock expected');
        });
    });
    ...
    describe(".ApplyEffect rock to home team player", function() {
        it("should result in the target player having the rock effect ", function(){
            validGame.WhosTurnIsIt = 0;
            validGame.HomeTeam.Id = 1;
            validGame.HomeTeam.Mana = 3
            validGame.HomeTeam.Pitch.push({Id : 1, IsRock: false});
            ...
            DeckHelper.findCardInDeck(1, modifiedGame.HomeTeam.Pitch)[0].should.have.property('IsRock', true);
        });
    });

    describe(".ApplyEffect rock to home team player not on pitch", function() {
        it("should result in the target player having the rock effect ", function(){
            validGame.WhosTurnIsIt = 0;
            validGame.HomeTeam.Id = 1;
            validGame.HomeTeam.Mana = 3
    ...
    describe(".ApplyEffect rock to away team player", function() {
        it("should result in the target player having the rock effect ", function(){
            validGame.WhosTurnIsIt = 0;
            validGame.AwayTeam.Id = 2;
            validGame.AwayTeam.Mana = 3
            validGame.AwayTeam.Pitch.push({Id : 2, IsRock: false});
            var validEffect = {};
            validEffect.Type = "Rock";
            validEffect.Target = 2;
            validEffect.TeamId = 2;
            modifiedGame = effect.applyEffect(validGame, validEffect);
            DeckHelper.findCardInDeck(2, modifiedGame.AwayTeam.Pitch)[0].should.have.property('IsRock', true);
        });
    });
});